...
import { Search, Calendar, User, ArrowRight, ChevronLeft, ChevronRight, Rss } from 'lucide-react';
import Link from 'next/link';
import Image from 'next/image';
import { Blog } from '@/lib/data/blogs';
import { useRouter, usePathname, useSearchParams } from 'next/navigation';

const List = ({ blogs, categories, currentPage, totalPages }: { blogs: Blog[], categories: string[], currentPage: number, totalPages: number }) => {
    ...
    const handleCategoryChange = (category: string) => {
        // Reset to page 1 when category changes to avoid 404/Empty states
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
                    <div className="w-full">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-brand-navy border border-brand-navy/20">
                            <Rss size={14} className="text-brand-green fill-brand-green" />
                            <span className="text-white text-xs font-black uppercase tracking-widest">Knowledge Hub</span>
                        </div>
                ...
                <div className="flex flex-wrap gap-3">
                    {categories.map((cat) => (
                        <button
                            ...
                            className={`px-6 py-2.5 rounded-full border text-sm font-bold transition-all ${activeCategory === cat
                                ? 'bg-brand-navy text-white border-brand-navy shadow-lg shadow-brand-navy/20'
                                : 'bg-white border-brand-border text-brand-navy hover:border-brand-green'
                                }`}
                        >
                            {cat}
                        </button>
                ...
                <section className="max-w-7xl mx-auto mb-20">
                    ...
                    >
                        ...
                        <div className="w-full lg:w-1/2 p-8 lg:p-16 text-white">
                            ...
                            <Link href={`/blog/${blogs[0].slug}`} className="inline-flex items-center gap-2 font-black uppercase tracking-widest text-sm text-brand-green group/btn">
